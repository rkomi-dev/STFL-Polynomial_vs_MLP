...
SSR_res_polif = sum(epsilon_T_arm.^2);
SSR_tot_polif = sum((data_test.LOAD - (mean(data_test.LOAD))).^2);
R2_polif = 1 - (SSR_res_polif / SSR_tot_polif);
...
fprintf('MAPE Polinomio + Fourier: %.2f%%\n', mape_arm);
fprintf('R^2 Polinomio + Fourier: %.2f%%\n', R2_polif);
...
phi_grid_poly = [ones(numel(X_surf), 1), X_surf(:), Y_surf(:), ...
    X_surf(:).^2, Y_surf(:).^2, ...
    X_surf(:).^3, Y_surf(:).^3, X_surf(:).^4, Y_surf(:).^4, ...
    X_surf(:).^5, Y_surf(:).^5];
...
title(sprintf('Polinomio 5° grado\n RMSE: %.2f - MAPE: %.2f%% - R2: %.4f', RMSE_finale_test, mape_poly, R2_poli));
grid on; view(3);
...
scatter3(w_avg_trainval, data_trainval.TIMESTAMP, data_trainval.LOAD, 5, 'b', 'filled', 'MarkerFaceAlpha', 0.05);
xlabel('Temperatura Media (°C)'); ylabel('Ora del giorno'); zlabel('Carico elettrico (MW)');
title(sprintf('Polinomio 5° grado + Fourier\n RMSE: %.2f - MAPE: %.2f%% - R2: %.4f', RMSE_arm_test, mape_arm, R2_polif));
grid on; view(3);
