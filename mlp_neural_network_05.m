...
X_test_mlp = [w_avg_test, time_sin_test, time_cos_test];

% Normalizzazione

[X_train_n, settings_x] = mapminmax(X_train_mlp'); 
X_test_n = mapminmax('apply', X_test_mlp', settings_x);
...
k = 5; % Numero di fold
punti_taglio = floor(linspace(n/2, n*0.9, k)); 
...
for i = 1:length(neuron_range)
    ...
    for f = 1:k
        ...
        X_test_fold = X_train_n(:, idx_fine_train+1 : idx_fine_test);
        y_test_fold = y_train_n(idx_fine_train+1 : idx_fine_test);
        ...
        y_test_pred_mw = mapminmax('reverse', net_fold(X_test_fold), settings_y);
        y_test_mw = mapminmax('reverse', y_test_fold, settings_y);
        rmse_test_fold(f) = sqrt(mean((y_test_mw - y_test_pred_mw).^2));
    end
    
    rmse_kfold_train_medio(i) = mean(rmse_train_fold);
    rmse_kfold_test_medio(i) = mean(rmse_test_fold);
    fprintf('Neuroni: %d | RMSE train Medio K-Fold: %.2f MW |RMSE test Medio K-Fold: %.2f MW\n', ...
        n_hid, rmse_kfold_train_medio(i), rmse_kfold_test_medio(i));
end
...
hold on
plot(neuron_range, rmse_kfold_test_medio, '-s', 'LineWidth', 1.5);
hold on
grid on
...
y_cap_mlp_test = mapminmax('reverse', net(X_test_n)', settings_y);

% Calcolo RMSE
RMSE_mlp_train = sqrt(mean((data_train.LOAD - y_cap_mlp_train).^2))
RMSE_mlp_test = sqrt(mean((data_test.LOAD - y_cap_mlp_test).^2))
