...
for i = 1:length(neuron_range)
    ...
    for f = 1:k
        ...
        net_fold.divideParam.trainRatio = 0.85;
        net_fold.divideParam.valRatio   = 0.15;
        net_fold.divideParam.testRatio  = 0; 
        net_fold.trainParam.max_fail = 10;
...
net_finale_25.divideParam.trainRatio = 0.85;
net_finale_25.divideParam.valRatio   = 0.15;
net_finale_25.divideParam.testRatio  = 0; 
net_finale_25.trainParam.max_fail = 10;
...
save('modello_finale.mat', 'net_finale_25', 'settings_x_25', 'settings_y_25');

%% boxplot dei residui
figure;
boxplot(errore_25, data_test.TIMESTAMP);
...
x_min = min([min(errore), min(errore_25)]);
x_max = max([max(errore), max(errore_25)]);
...
%% Andamento temporale: 2 settimane casuali
num_ore_test = length(data_test.LOAD);
ore_settimana = 168;

start1 = randi([1, num_ore_test - ore_settimana]);
start2 = randi([1, num_ore_test - ore_settimana]);

while abs(start1 - start2) < ore_settimana
    start2 = randi([1, num_ore_test - ore_settimana]);
end

figure('Color', 'w', 'Name', 'Analisi Ciclicità Settimanale Casuale');
t_asse = 1:ore_settimana; 

% --- SETTIMANA CASUALE 1 ---
subplot(2, 1, 1)
idx1 = start1 : (start1 + ore_settimana - 1);
plot(t_asse, data_test.LOAD(idx1), 'k', 'LineWidth', 1.2); hold on;
plot(t_asse, y_test_pred_mw_25(idx1), 'r--', 'LineWidth', 1.1);
xlabel('Tempo (ore)');
ylabel('Carico (MW)');
title('Confronto Temporale: Settimana Casuale 1');
for d = 24:24:144
    xline(d);
end
...
% --- SETTIMANA CASUALE 2 ---
subplot(2, 1, 2)
idx2 = start2 : (start2 + ore_settimana - 1);
plot(t_asse, data_test.LOAD(idx2), 'k', 'LineWidth', 1.2); hold on;
plot(t_asse, y_test_pred_mw_25(idx2), 'r--', 'LineWidth', 1.1);
xlabel('Tempo (ore)');
ylabel('Carico (MW)');
title('Confronto Temporale: Settimana Casuale 2');
for d = 24:24:144
    xline(d);
end
